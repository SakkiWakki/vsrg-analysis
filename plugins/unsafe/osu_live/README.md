...
1. **Native reader** (preferred). A small Rust PyO3 extension
   (`native/`) that scans osu!'s memory via `process_vm_readv` on
   Linux/wine and reads the gameplay struct directly. ~40 µs per read,
   no server, no port.
...
```sh
pip install maturin
cd plugins/unsafe/osu_live/native
maturin develop
```
...
from tosu (see `native/src/signatures.rs` for provenance); the
...
`native/src/signatures.rs` and rebuild with `maturin develop`. The
