"""The curve assembler reproduces the position + rotation axes of
arrow_effects.note_offsets, byte-equal (ULP), across a battery of real
percent dicts (single mods, combos, numbered per-column variants). This
is the proof that the live note_offsets position core can run on curves.

The zoom / alpha / glow / perspective (hallway, confusiony) families are
not curve-ported; assemble() delegates them to the kernels, so they are
out of scope here and excluded from the battery."""
import numpy as np
import pytest
