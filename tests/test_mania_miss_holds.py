...
def _model_for_holds(note_rows, note_cols, miss_holds):
    ...
    model.miss_hold_ln_heads_ms = np.array(
        [h[0] for h in miss_holds], dtype=np.int64)
    model.miss_hold_cols = np.array([h[1] for h in miss_holds],
                                    dtype=np.int32)
    model.miss_hold_press = np.array([h[2] for h in miss_holds],
                                     dtype=np.float64)
    model.miss_hold_release = np.array([h[3] for h in miss_holds],
                                       dtype=np.float64)
    return model
...
def test_same_ln_only_first_miss_extends_from_head():
    ...
    assert model.miss_first_hold.tolist() == [0, -1]
