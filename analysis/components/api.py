...
class GameState(Protocol):
    ...
    def audio_status(self) -> tuple[int, str]:
        """Return ``(count, last_status_string)`` of PortAudio callback
        status flags seen since the audio engine started. ``count > 0``
        means the audio thread has missed at least one block deadline
        (output underflow); the string is the most recent flag.

        Surfaces without an audio engine return ``(0, '')`` rather
        than raising, so HUD components can render a clean baseline."""
        ...
