...
def assemble(percents, cols, y_offset, t_now, beat_now, keycount,
    ...
    """A `NoteOffsets` drop-in for `arrow_effects.note_offsets`: the
    position + rotation axes (dx, dy, z, rotation_deg, rot_x, rot_y) come
    from the curve assembler; the zoom / alpha / glow families and the
    perspective reprojections (hallway, confusiony) stay on the kernels
    until those families are curve-ported. Output is byte-equal to
    `note_offsets`.

    Signature mirrors `note_offsets` so the two are interchangeable."""
    ...
    dx = _sum(dx_curves(percents, cols, keycount, arrow_size, beat_now),
              y_offset, ctx)
    # Perspective X (hallway / confusiony) not yet curve-ported.
    dx = dx + ae._perspective_dx(percents, cols, y_offset, beat_now,
                                 keycount, arrow_size)
    ...
    z = rot_x = rot_y = None
    if project_3d:
        z = _sum(z_curves(percents, cols, keycount, arrow_size, beat_now),
                 y_offset, ctx)
        z = np.broadcast_to(z, (n,)).astype(np.float64)
        rot_x, rot_y = ae._note_tilt(percents, y_offset, n)
    zoom = ae._zoom(percents, cols, y_offset, t_now, beat_now, keycount,
                    arrow_size, n, z_push=z)
    vis_y = y_offset + ae._tipsy_dy(percents, cols, t_now, keycount, arrow_size)
    alpha = ae._alpha(percents, cols, vis_y, t_now)
    glow = None
    if _g(percents, 'stealthglow'):
        glow = ae.stealthglow_amount(_g(percents, 'stealthglow'), vis_y)
